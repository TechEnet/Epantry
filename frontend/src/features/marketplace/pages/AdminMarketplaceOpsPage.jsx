...
function ActionCard({ icon: Icon, title, description, to, label }) {
  return (
    <div className="rounded-[22px] border border-stone-200 bg-white p-5 shadow-sm">
      ...
      <h2 className="mt-4 font-black text-stone-950">{title}</h2>
    ...
    >
      ...
      <section className="mt-5 rounded-[22px] border border-amber-200 bg-amber-50 p-5">
        ...
        <p className="mt-2 text-sm leading-6 text-amber-900">
          Internal Marketplace Ops does not receive an implicit tenant
          impersonation path. Host commercial writes continue through the
          authenticated Host workspace.
        </p>
      </section>
    </AdminShell>
  );
